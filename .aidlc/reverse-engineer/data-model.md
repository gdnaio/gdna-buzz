...
### Batch 2d data model (agent surface: buzz-acp, buzz-agent, buzz-dev-mcp, buzz-cli)

2d contains **no persistence**. Every crate in this batch is stateless across process restarts:
zero SQL, zero schema, and the only files written are an OAuth token cache and process-scoped temp
directories. The data model here is therefore in-memory state machines plus wire representations,
and the interesting findings are about invariants that the type system does not enforce.

| ID | Finding | Location |
|---|---|---|
| DM-2d-1 | **No database and no persisted state across all four crates.** `buzz-agent`'s six core files contain zero `fs::write`/`File::create`/`sqlite` matches — session state dies with the process. The only writes in the batch are `buzz-agent`'s OAuth token cache (`auth.rs:455-467`) and `buzz-dev-mcp`'s two process-scoped temp directories. | verified by grep across `crates/buzz-agent/src`, `crates/buzz-acp/src` |
| DM-2d-2 | **The `Session` record has 17 fields and one non-obvious lifetime rule: history is moved *out* while a prompt runs.** `std::mem::take` at `lib.rs:807` empties the map's copy for the duration of the turn, and it is written back at `:709`. So a panic mid-turn leaves the session with `busy: true` and **no history** — the context is gone, not just the turn (`:704-705`, no `catch_unwind` anywhere). `cancel_tx` is replaced with a fresh channel on each `acquire_session` (`:789-790`) so a stale cancel cannot kill the next turn; `steer_tx` is created per turn and cleared after (`:802-803`, `:708`). `Session::id` duplicates the `HashMap` key (both set from the same value at `:411-412`) and can never legitimately diverge, but nothing enforces that. | `crates/buzz-agent/src/lib.rs:64-103`, `:704-712`, `:785-819` |
| DM-2d-3 | **Sessions have no terminal state.** `sessions.remove` returns zero matches in `lib.rs`, so a session — and the `Arc<McpRegistry>` of live child processes it holds (`:66`, spawned `:390`) — survives until process exit. `max_sessions` defaults to `usize::MAX` (`config.rs:812`). Shutdown only broadcasts cancel (`:178-180`); there is no `session/close`. | `crates/buzz-agent/src/lib.rs:64-66`, `:178-180`; `config.rs:812` |
| DM-2d-4 | **Two independent size measures on the same history, dispatched through one function — and this is the most heavily tested invariant in the batch.** `estimated_bytes` reports real wire size (`types.rs:29-36`, `:70-72`) and drives `truncate_history`; `context_pressure_bytes` substitutes a flat `IMAGE_CONTEXT_TOKEN_EQUIV = 16 KiB` for image payloads (`types.rs:14`, `:42-47`) and drives the handoff gate and the usage baseline. Both route through `size_with` (`:83-104`). Four tests pin the split, including one asserting a single image does not trip the shipped 167,232-byte threshold (`:292`, `:303`, `:326`, `:346`). | `crates/buzz-agent/src/types.rs:14`, `:29-47`, `:83-104`, `:292-352` |
| DM-2d-5 | **`provider_id` is a non-`Option` `String` on both `ToolCall` and `ToolResult`** (`types.rs:107-118`), which is the type-level encoding of the `tool_use ↔ tool_result` pairing. Synthetic results always copy the originating id (`agent.rs:703-709`) and the built-in `load_skill` result is re-stamped with it (`:325`). But nothing in the type system enforces role alternation or that every `Assistant.tool_calls` entry has a matching `ToolResult` — that pairing is maintained procedurally by `append_results` pushing one result per call in original order (`agent.rs:350-368`), with index correspondence between `Vec<Option<ToolResult>>` and `calls` as the load-bearing invariant across the `JoinSet` (`:296-297`, `:431`). | `crates/buzz-agent/src/types.rs:107-118`; `agent.rs:296-297`, `:350-368` |
| DM-2d-6 | **Every LLM request body is hand-built `serde_json::Value`, and one `HistoryItem` maps three different ways per provider.** No provider request type is serde-derived. `HistoryItem::ToolResult` becomes a batched trailing user message for Anthropic (`llm.rs:717-719`, flushed `:686-690`), a contiguous run of `role:"tool"` messages for OpenAI Chat (`:811-813`), and `function_call_output` items for Responses (`:906-910`). Two ordering invariants live in the builders rather than the types: Anthropic requires all `tool_result` blocks for one assistant turn in a single following user message, and OpenAI Chat requires the `role:"tool"` run to stay contiguous — which is why images are deferred into a trailing user message via `pending_images` (`:778-782`, extended `:814`, flushed `:818`), regression-tested at `:2541`. Tool `arguments` are a JSON **object** inline for Anthropic and a JSON **string** for both OpenAI dialects (`:705-706` vs `:798-804`, `:896-903`), converging on one normalizer, `make_tool_call` (`:1248-1267`). | `crates/buzz-agent/src/llm.rs:676-962`, `:1248-1267` |
| DM-2d-7 | **`Config` is a 27-field flat struct whose invariants are `from_env` post-conditions, not type invariants.** All 27 fields are `pub` while `validate` is private (`config.rs:687-739`, `:878`), and `Config::for_discovery` (`:845-876`) constructs a value that would **fail** `validate` — `max_output_tokens: 1` (`:856`), `mcp_max_restart_attempts: 0` (`:860`), `mcp_restart_base_ms: 0` (`:861`) — and never calls it. The odd `max_context_tokens: 200_001` (`:867`) exists only to satisfy a check that never runs. `Config` carries no `Serialize`/`Deserialize`, so it cannot be accidentally serialized into a payload — but it does derive `Debug` with `api_key` as a normal field, so a single `debug!(?cfg)` added anywhere would print the key with no compiler or lint objection. The 27th field, added by `16d4ec33`, is `prefer_mesh_for_auto: bool` (`:734`) — a plain `bool` with no invariant checked anywhere in `validate`. | `crates/buzz-agent/src/config.rs:687-739`, `:845-876`, `:878` |
| DM-2d-21 | **New in `16d4ec33`: `Llm` gained a fourth field and three new private types to run mesh "Auto" collective routing entirely in-process.** `Llm` (`llm.rs:88-105`) now holds `mesh_auto_state: Mutex<MeshAutoState>` alongside its original three fields — the file's first `tokio::sync::Mutex`. `MeshAutoState` (`:73-79`) is a 4-field hysteresis record (`last_checked: Option<Instant>`, `consecutive_available: u8`, `collective_enabled: bool`, `cooldown_until: Option<Instant>`); `MeshCatalogObservation` (`:40-45`) is a 3-variant enum (`Available`/`Unavailable`/`Unknown`) where `Unknown` is deliberately treated as *no evidence* rather than absence, so a failed probe cannot flip the decision. `PostError` (`:1344-1347`, 2 variants: `Agent(AgentError)`, `MeshFallback(String)`) is a new internal error shape that lets `post` (`:1390-1517`) and `post_openai` (`:599-605`) carry a mesh-specific signal out of the HTTP layer without widening the public `AgentError` — `into_agent()` (`:1350-1356`) and `From<AgentError>` (`:1358-1362`) are the two directions of collapse. All three types are private to `llm.rs`. | `crates/buzz-agent/src/llm.rs:40-105`, `:1344-1362`, `:1390-1517` |
| DM-2d-8 | **`ThinkingEffort`'s numeric discriminants are load-bearing, not just its `Ord`.** `resolve_openai_effort` computes `(e as i32 - requested as i32).unsigned_abs()` (`config.rs:487`), which requires the variants to be contiguous 0..=6 in declaration order (`:19-27`). `thinking_effort_ord_ordering` (`:1387`) asserts the `Ord` relation but never the cast values, so inserting a variant mid-enum would change every fallback distance while the test stayed green. | `crates/buzz-agent/src/config.rs:19-27`, `:487` |
| DM-2d-9 | **`WireMsg` has exactly one variant**, making the writer's `let WireMsg::Notify(v) = msg;` (`wire.rs:223`) infallible — a forward-compat placeholder rather than a discriminant in use. `ContentBlock::Unsupported` (`types.rs:210-221`) is the mirror image: `#[serde(other)]` means an unknown block type deserializes successfully and fails only later in `prompt_to_text` (`agent.rs:623-628`) — and asymmetrically, a bad block in the *initial* prompt aborts the turn while the same block in a steer is dropped with a `warn!` (`:275-278`). | `crates/buzz-agent/src/wire.rs:223`; `types.rs:210-221` |
| DM-2d-10 | **`RunCtx` owns nothing** — all 17 fields are borrows of locals in `run_prompt` (`agent.rs:24-64`, `lib.rs:674-693`), and `acquire_session` returns a **13-element anonymous tuple** (`lib.rs:764-783`) destructured into 13 named locals at the call site (`:632-652`). Adding one piece of session state means four coupled edits — signature, tuple literal (`:806-819`), destructuring pattern, write-back block (`:704-712`) — with no compiler help on ordering, since several fields share `Option<String>` / `Option<u64>`. | `crates/buzz-agent/src/agent.rs:24-64`; `lib.rs:764-783` |
| DM-2d-11 | **`buzz-acp`'s queue state includes three unbounded maps.** `cancelled_batches` (`queue.rs:543-546`), `withheld_native_steer`, and `in_flight_batch_sizes` have no `MAX_BATCH_EVENTS` analogue and are untouched by `compact_expired_state` (`:809-817`), so `cancelled_batches` grows the merged prompt without limit. `retry_after` is the coupling that makes ordering matter (see BR-155). | `crates/buzz-acp/src/queue.rs:543-546`, `:809-817` |
| DM-2d-12 | **The OAuth token cache is the batch's only durable artifact, and its format is asserted only by round-trip.** `$HOME/.config/buzz-agent/oauth/<namespace>/<sha256>.json` (`auth.rs:455-467`) holds `CachedToken` — access token, refresh token, expiry — pretty-printed in cleartext (`:110-118`, `:192`). The filename hashes the client id, scopes and discovery URL (`:446-454`), so a change to any of those orphans every existing cache silently. `CachedToken` derives `Debug` with no redacting impl. The two on-disk-format tests re-serialize the production struct (`:750-756`, `:786-792`), so a field rename keeps them green while orphaning real caches. | `crates/buzz-agent/src/auth.rs:110-118`, `:446-467` |
| DM-2d-13 | **`buzz-cli`'s filters are untyped `serde_json::Value` throughout**, never a typed struct (`client.rs:683-713`, `:767-801`). This is the root cause of the p-gate class of bugs: a missing `kinds` or a misspelled tag key is a relay-side 4xx rather than a compile error. Pagination state is written *into* the filter as a composite cursor — `limit`, `until`, `before_id` (`:690`, `:515-516`) — with the id validated as 64 ASCII hex before reuse (`:511-514`) and two tests pinning the cursor semantics (`:2305-2331`). | `crates/buzz-cli/src/client.rs:498-519`, `:683-801` |
| DM-2d-14 | **Relay responses are `String`, not typed results.** `BuzzClient::submit_event` and friends return the raw body (`client.rs:863`), pushing JSON re-parsing into 47 `normalize_write_response` call sites. The single exception is `BlobDescriptor` (`client.rs:12-38`), the only serde-typed relay response in the crate, which `build_imeta_tag` (`:40-62`) flattens into a NIP-92 `imeta` tag array. `normalize_events` (`:1307-1323`) is the only sig-stripping projection and defaults missing fields to `""`/`0`/`[]`. | `crates/buzz-cli/src/client.rs:12-62`, `:863`, `:1307-1323` |
| DM-2d-15 | **`buzz-cli` has three `parse_events` implementations with the same name and two different failure policies.** `repos.rs:11-14` and `notes.rs:156-159` are byte-identical and strict — one bad event fails the whole response; `mem.rs:114-136` is deliberately lenient, skipping undeserializable entries so a corrupt event cannot deny-of-service a listing (`:118-128`). Nothing in `client.rs` offers either. `d`-tag extraction is likewise threefold: the shared `client::extract_d_tag` (used by `workflows.rs:24`), a hand-rolled `repo_id_from_event` (`repos.rs:33-45`), and a third variant comparing `t.kind().to_string() == "d"` (`mem.rs:212-220`) — the most fragile of the three. | `crates/buzz-cli/src/commands/repos.rs:11-14`, `:33-45`; `mem.rs:114-136`, `:212-220` |
| DM-2d-16 | **`channels.rs` contains three independent kind-39000 parsers whose outputs disagree.** `extract_channel_metadata` (`:16-24`) yields 4 fields; `ChannelSummary::from_event` (`:166-211`) yields 8; an inline `--visibility` post-filter (`:68-89`) re-derives the single-element-tag rule a third time. So `channels get` returns strictly less than `channels search` for the *same event* — no `topic`, `purpose`, `archived`, `channel_type` or `visibility` — and the two commands disagree on the archived default (`channels search` excludes archived, `channels list` has no such filter). The vocabulary splits too: `--visibility open` maps to the wire value `public` (`:71-73`) while `ChannelSummary.visibility` reports `"public"` verbatim (`:183`). | `crates/buzz-cli/src/commands/channels.rs:16-24`, `:68-89`, `:166-211` |
| DM-2d-17 | **Engram `d` tags are HMAC-derived, not slug-derived**, which makes memory addressing capability-scoped rather than permission-checked: `d_tag = HMAC(K_c, slug)` over domain `agent-memory/v1/d-tag` (`crates/buzz-core/src/engram.rs:24`, `:144-154`), where `K_c` is the agent↔owner NIP-44 conversation key. Without that key a reader cannot even construct the filter. Slugs are `core` or `mem/` + `[a-z0-9][a-z0-9_-]{0,63}` segments, ≤ 255 bytes (`engram.rs:66-90`), so a slug is safe to print verbatim by construction. Memory "files" are virtual — one slug is one virtual file (`mem.rs:610-613`) held in a kind-30174 event, never on disk. | `crates/buzz-core/src/engram.rs:24`, `:66-90`, `:144-154` |
| DM-2d-18 | **The agent-draft payload is a three-layer envelope with two hardcoded fields.** `CreateAgentDraft`/`UpdateAgentDraft` → `ManagementRequest<T>` (with a UUIDv4 `request_id`) → `ObserverEvent<T>`, encrypted to the owner and published as kind 24200 (`agent_management.rs:15-61`, `:107-121`). `ObserverEvent.seq` is hardcoded `0` (`:96`). Size caps are 120 chars for names, 20,000 for prompts, 300 for other optional fields (`:10-11`, `:83-85`), all counted in **characters** not bytes. The frame carries `p`, observer-agent and observer-frame tags and deliberately no `h` tag, asserted by `create_is_owner_encrypted_and_matches_desktop_contract` (`:188-241`). | `crates/buzz-cli/src/agent_management.rs:10-11`, `:15-61`, `:83-121` |
| DM-2d-19 | **The kind registry recount after the post-sync reconciliation: 130 kind constants, `ALL_KINDS` with 127 entries, three deliberate exclusions.** Verified by set difference: `KIND_AUTH` (`kind.rs:77`), `KIND_NOSTR_IDENTITY_BINDING` (`:81`), `KIND_PUSH_LEASE` (`:109`). The 23-commit sync range added three public classifier functions and a large doc block on `KIND_PERSONA` but **no new kind integers**, so these counts are unchanged from the original analysis. | `crates/buzz-core/src/kind.rs:77`, `:81`, `:109`, `:567-693` |
| DM-2d-20 | **`buzz-db`'s `EventQuery` now carries 19 filter fields, not 18** — `persona_reader` was added at `event.rs:88` by the sync, and the optional-predicate count in `query_events` went from 11 to 12. Recorded here because the field count is cited in the `buzz-db` module section. | `crates/buzz-db/src/event.rs:88` |

Positive controls in 2d data modelling: `buzz-agent`'s two-measure split (DM-2d-4) is a genuine
design win — the comment at `types.rs:5-13` explains why a flat 16 KiB image equivalent is a
correct *ceiling* over the ~2K visual tokens providers actually bill, and four tests hold the line.
Pure policy math is deliberately extracted to free functions so it is testable without a `RunCtx`,
stated at `handoff.rs:344-346` and applied to `token_threshold`, `byte_fallback_threshold`,
`handoff_prompt_budget_bytes` and `estimate_tokens_from_bytes`, all unit-tested (`:378-428`).
`make_tool_call` (`llm.rs:1248-1267`) is a single normalizer for three dialects that rejects an empty
id or name, coerces `Value::Null` to `{}`, and rejects any other JSON type. And `sum_usage`
(`llm.rs:1107-1118`) carefully distinguishes "no usage reported" from "usage was zero" — with nine
tests covering the input side and six the output side.
...
| `models_cache` | `tokio::sync::OnceCell<Vec<ModelEntry>>` | `lib.rs:61` | Populated only on *successful* Databricks discovery via `get_or_try_init` (`lib.rs:318`) so an error leaves the cell empty and the next `session/new` retries. The error path returns `discovery_failure_fallback` without caching it (`lib.rs:324`); since `8eb6e3eb` that value carries `DATABRICKS_V2_KNOWN_MODELS.len() + 1` entries for `DatabricksV2` — the known list plus the trimmed configured model (`catalog.rs:61-76`) |
...
### Aspect: Data Model
This group owns three shapes: the resolved configuration struct (`config.rs`), the on-the-wire request/response representations for four provider dialects (`llm.rs`), and — new since `16d4ec33` — a small in-process state machine for mesh "Auto" collective routing (`llm.rs:40-79`). Nothing here is a serde-derived request type — every provider body is hand-built `serde_json::Value`.

#### Resolved configuration struct
`Config` (`config.rs:687-739`) is a flat **27**-field struct deriving only `Debug, Clone` (`config.rs:686`). It carries no `Serialize`/`Deserialize` — grep for `derive(Serialize` / `derive(Deserialize` in `config.rs` returned zero matches, so a `Config` cannot be accidentally serialized into a payload.
...
| `llm_timeout` | `Duration` | `config.rs:692`; becomes `reqwest` **read** timeout only (`llm.rs:111`) |
...
| `max_context_tokens` | `u64` | `config.rs:712` — deliberately `u64` so it can be compared to provider-reported `input_tokens: Option<u64>` |
| `max_handoffs`, `max_parallel_tools`, `hook_timeout`, `stop_max_rejections` | `usize`/`Duration`/`u32` | `config.rs:713-718` |
| `hook_servers` | `HookServers` | `config.rs:722` |
...
| `openai_api` | `OpenAiApi` | `config.rs:728` |
| `prefer_mesh_for_auto` | `bool` | `config.rs:734` — **new**; the only `llm.rs`-consumed feature flag on `Config`. Read at `llm.rs:412` to gate the whole mesh-Auto policy |
| `hints_enabled` | `bool` | `config.rs:735` |
| `thinking_effort` | `Option<ThinkingEffort>` | `config.rs:738`; `None` means "send no thinking config at all" |

#### Config invariants and where they are (not) enforced
`Config::validate` (`config.rs:878-959`) is the only invariant gate, and it is called from exactly one place: `from_env` at `config.rs:835`.

| Invariant | Site |
|---|---|
| `max_output_tokens >= 1` | `config.rs:884-886` |
| `max_context_tokens > max_output_tokens` | `config.rs:887-892` |
| `max_history_bytes >= 4096` | `config.rs:893-897` |
| `max_history_bytes >= MAX_PROMPT_BYTES` (1 MiB, `config.rs:638`) | `config.rs:898-903` |
| `max_line_bytes >= 1024` | `config.rs:904-908` |
| `1024 <= max_tool_result_text_bytes <= MAX_TOOL_RESULT_BYTES` (8 MiB, `config.rs:643`) | `config.rs:909-915` |
| `llm_timeout`, `tool_timeout`, `mcp_init_timeout` each `>= 1s` | `config.rs:916-924` |
| `max_parallel_tools >= 1`, `mcp_max_restart_attempts >= 1`, `mcp_restart_base_ms >= 1`, `mcp_restart_max_ms >= mcp_restart_base_ms` | `config.rs:925-939` |
| `thinking_effort != none\|minimal` when `provider == Anthropic` | `config.rs:947-957` |

`prefer_mesh_for_auto` adds **no** invariant: nothing in `validate` (`config.rs:878-959`) checks that it is only set for `Provider::OpenAi`, or that `model`/`OPENAI_COMPAT_MODEL` is `auto`. Both consistency conditions are re-tested per request instead, at `llm.rs:411-415`.

`Config::for_discovery` (`config.rs:845-876`) constructs a `Config` that **would fail `validate()`**: it sets `max_output_tokens: 1` (`config.rs:856`), `mcp_max_restart_attempts: 0` (`config.rs:860`), `mcp_restart_base_ms: 0` (`config.rs:861`) — all rejected by `config.rs:928-933` — and never calls `validate()`. The odd `max_context_tokens: 200_001` (`config.rs:867`) exists only to satisfy the `>` comparison at `config.rs:887` if validation were ever added. The discovery fixture also pins `prefer_mesh_for_auto: false` (`config.rs:854`), so catalog discovery can never trip the mesh policy. There is no type-level distinction between a validated and an unvalidated `Config`, so the invariants at `config.rs:884-957` are not struct invariants, only `from_env` post-conditions.
...
| `HookServers` | `None`, `All`, `Only(Vec<String>)` | `Debug, Clone` (`config.rs:1062`) | `config.rs:1063-1067` |
| `DatabricksV2Route` | `OpenAiResponses`, `AnthropicMessages`, `MlflowChatCompletions` | `Debug, Clone, Copy, PartialEq, Eq` (`llm.rs:81`) — private to `llm.rs` | `llm.rs:82-87` |
| `MeshCatalogObservation` | `Available`, `Unavailable`, `Unknown` | `Debug, Clone, Copy, PartialEq, Eq` (`llm.rs:40`) — private to `llm.rs` | `llm.rs:41-45` |
| `PostError` | `Agent(AgentError)`, `MeshFallback(String)` | `Debug` only (`llm.rs:1343`) — private to `llm.rs` | `llm.rs:1344-1347` |

`ThinkingEffort`'s **numeric discriminants** are load-bearing, not just its `Ord`: `resolve_openai_effort` computes `(e as i32 - requested as i32).unsigned_abs()` at `config.rs:495`. That distance metric depends on the variants being 0..=6 in declaration order (`config.rs:19-27`) with no explicit discriminants. `thinking_effort_ord_ordering` (`config.rs:1395-1406`) asserts the `Ord` relation but never the cast values, so a reordering that preserved `Ord` while changing gaps would not be caught.

`MeshCatalogObservation` is a deliberate three-valued type rather than a `bool`: `Unknown` (`llm.rs:44`) means "the probe did not answer", and the state machine treats it as *no evidence* rather than as absence — `llm.rs:452-456` documents this and the `Unknown` arm at `llm.rs:457` is an explicit no-op.

#### Mesh Auto routing state
`MeshAutoState` (`llm.rs:74-79`, `#[derive(Debug, Default)]` at `llm.rs:73`) is the only mutable per-process state in this group besides the `auto_upgraded` latch:

| Field | Type | Meaning | Site |
|---|---|---|---|
| `last_checked` | `Option<tokio::time::Instant>` | when the `/models` catalog was last probed; gates the TTL | `llm.rs:75` |
| `consecutive_available` | `u8` | how many consecutive probes saw a collective-capable mesh; the hysteresis counter | `llm.rs:76` |
| `collective_enabled` | `bool` | the confirmed decision — `true` routes to `mesh`, `false` to `auto` | `llm.rs:77` |
| `cooldown_until` | `Option<Instant>` | suppresses both probing and collective selection after a mesh-specific failure | `llm.rs:78` |

It is held behind a `tokio::sync::Mutex` on `Llm` (`llm.rs:98`) — the file's first async lock; `tokio::sync::Mutex` and `tokio::time::Instant` are imported for it at `llm.rs:7-8`. Transitions live in two methods: `resolve_openai_model` (`llm.rs:410-469`) for the read/TTL/observe path and `cool_down_collective` (`llm.rs:397-404`) for the failure path, which resets all four fields at once (`llm.rs:400-403`).

The tuning constants are module-level `const`s with no env override (`llm.rs:28-34`):

| Constant | Value | Site |
|---|---|---|
| `MESH_VIRTUAL_MODEL_ID` | `"mesh"` | `llm.rs:28` |
| `MESH_AUTO_MODEL_ID` | `"auto"` | `llm.rs:29` |
| `MESH_AUTO_CATALOG_TTL` | 5 s | `llm.rs:30` |
| `MESH_AUTO_CATALOG_TIMEOUT` | 2 s | `llm.rs:31` |
| `MESH_AUTO_COOLDOWN` | 30 s | `llm.rs:32` |
| `MESH_AUTO_ENABLE_OBSERVATIONS` | 2 | `llm.rs:33` |
| `MESH_MOA_UNAVAILABLE_MESSAGE` | `"MoA requires ≥2 models available in the mesh"` | `llm.rs:34` |

`PostError` (`llm.rs:1344-1347`) is the new internal error shape that carries the mesh signal out of the HTTP layer without widening `AgentError`. `into_agent` (`llm.rs:1350-1356`) collapses `MeshFallback(detail)` into `AgentError::Llm(detail)`, and `From<AgentError>` (`llm.rs:1358-1362`) lifts in the other direction, so only the two functions that need the distinction — `post` (`llm.rs:1390-1517`) and `openai_request` (`llm.rs:344-395`) — ever see it. `types.rs:213-220`'s `AgentError` is unchanged by these commits.

#### `Llm` client state
`Llm` (`llm.rs:88-105`) holds **four** fields: a `reqwest::Client` (`llm.rs:89`), an `AtomicBool auto_upgraded` (a one-way sticky latch for the chat→responses upgrade, `llm.rs:90-94`), `mesh_auto_state: Mutex<MeshAutoState>` (`llm.rs:95-98`), and `auth: Arc<dyn TokenSource>` (`llm.rs:99-104`). The `auth` field is populated even for `Provider::Anthropic`, where it is never read for inference — documented at `llm.rs:1521-1523` as a way to keep the field non-`Option`. It *is* now read on one non-Anthropic-only path: the mesh catalog probe calls `self.auth.bearer()` at `llm.rs:474`.

`type OpenAiParse = fn(Value) -> Result<LlmResponse, AgentError>` (`llm.rs:38`) is the parser half of every `(body, parser)` pair. The name is a misnomer: the DatabricksV2 Anthropic route casts `parse_anthropic as OpenAiParse` at `llm.rs:199` (and again in the summarizer at `llm.rs:309`).

#### Request bodies per provider (all `serde_json::Value`, no typed structs)
| Builder | Top-level keys | Site |
|---|---|---|
| `anthropic_body` | `model`, `max_tokens`, `system`, `messages`, optional `thinking`, optional `output_config`, optional `tools` | `llm.rs:676-747`; base object at `llm.rs:730-731` |
| `openai_body` (Chat Completions) | `model`, `stream: false`, `max_completion_tokens`, `messages`, optional `reasoning_effort`, optional `tools` + `tool_choice: "auto"` | `llm.rs:761-839`; base object at `llm.rs:828-829` |
| `responses_body` (Responses API) | `model`, `instructions`, `max_output_tokens`, `input`, optional `reasoning: {effort}`, optional `tools` + `tool_choice: "auto"` | `llm.rs:874-962`; base object at `llm.rs:943-948` |

The `model` value these builders receive is no longer always the caller's `effective_model`. For `Provider::OpenAi` with `prefer_mesh_for_auto`, `openai_request` resolves a `request_model` first (`llm.rs:354`) and threads it into the builder closure (`llm.rs:359`, closure parameter at `llm.rs:352`), so the serialized `"model"` can read `"mesh"` for a request the session asked for as `"auto"`. `posted_models` (`llm.rs:1817-1824`) is the test helper that reads that field back off the wire.

Streaming vs non-streaming: only the Chat Completions shape carries an explicit `"stream": false` (`llm.rs:828`, and again in the summarizer bodies at `llm.rs:272` and `llm.rs:314`). `anthropic_body` and `responses_body` emit **no** `stream` key at all (`llm.rs:730-731`, `llm.rs:943-948`), relying on each API's non-streaming default. There is no SSE/streaming parse path in this file — grep for `text/event-stream` and `data:` frame handling in `llm.rs` returned zero matches, consistent with the "non-streaming" claim in `AGENTS.md:50` and `crates/buzz-agent/README.md:255`.
...
|---|---|---|---|
| `User(text)` | `{role:"user", content:[{type:"text",text}]}` (`llm.rs:695-696`) | `{role:"user", content: text}` — bare string (`llm.rs:788`) | `{role:"user", content:[{type:"input_text",text}]}` (`llm.rs:885-888`) |
| `Assistant{text, tool_calls}` | `content[]` of `text` + `tool_use` blocks; **entire turn skipped when both empty** (`llm.rs:708-714`) | one message with `content: text` plus `tool_calls[]` (`llm.rs:790-808`) | separate `{role:"assistant", content:[{type:"output_text"}]}` (emitted only when `text` non-empty, `llm.rs:890-895`) then one `function_call` item per call |
| `ToolResult(r)` | `{type:"tool_result", tool_use_id, content, is_error}` accumulated into a *single* trailing user message (`llm.rs:717-719`, flushed at `llm.rs:686-690`) | `{role:"tool", tool_call_id, content: <text>}` (`llm.rs:811-813`) | `{type:"function_call_output", call_id, output}` (`llm.rs:906-910`) |

Two ordering invariants are encoded in the builders rather than the types:
- Anthropic requires all `tool_result` blocks answering one assistant turn to arrive in one immediately-following user message; `anthropic_body` batches them via the `pending`/`flush` closure (`llm.rs:685-690`, flushed at `llm.rs:694`, `llm.rs:699`, `llm.rs:722`).
- OpenAI Chat requires the run of `role:"tool"` messages to stay contiguous, so images are deferred into a single trailing `role:"user"` message via `pending_images` (`llm.rs:778-782`, extended at `llm.rs:814`, flushed at `llm.rs:818`). The rationale — an OpenAI→Anthropic translating frontend such as Databricks model serving — is documented at `llm.rs:770-777` and regression-tested by `openai_parallel_image_tool_results_stay_contiguous` (`llm.rs:2541`).
- Responses requires each `function_call` to precede its `function_call_output`; this is asserted only as a comment block (`llm.rs:867-872`) plus the test `responses_body_replay_emits_function_call_before_output` (`llm.rs:2310`), relying on `HistoryItem` insertion order rather than a type constraint.
...
|---|---|---|
| Anthropic `tool_use` | JSON **object** inline as `input` | `llm.rs:705-706` |
| OpenAI Chat `tool_calls[]` | JSON **string** under `function.arguments`, serialized with `unwrap_or_else(\|_\| "{}")` | `llm.rs:798-804` |
| Responses `function_call` | JSON **string** under `arguments`, same fallback | `llm.rs:896-903` |

On the parse side, all three converge on `make_tool_call` (`llm.rs:1248-1267`), which is the single normalizer: it rejects an empty `id` **or** empty `name` (`llm.rs:1249-1251`), passes `Value::Object` through (`llm.rs:1253`), coerces `Value::Null` to `{}` (`llm.rs:1254`), and rejects any other JSON type (`llm.rs:1255-1259`).

There is now a *fourth*, degenerate encoding the parsers do **not** understand: some mesh reducer models emit tool calls as plain text markup rather than structured `tool_calls[]`. `looks_like_unstructured_tool_call` (`llm.rs:66-72`) is a text classifier for that shape — `<|tool_call`, `<tool_call`, or `[tool_call]` after `trim_start().to_ascii_lowercase()` — and it drives a retry rather than a parse (`llm.rs:373-387`). No `ToolCall` is ever built from it.

Tool definitions (`ToolDef`, `types.rs:167-171`) are also reshaped three ways: Anthropic `{name, description, input_schema}` (`llm.rs:723-728`), Chat `{type:"function", function:{name, description, parameters}}` (`llm.rs:822-829`), Responses **flat** `{type:"function", name, description, parameters}` (`llm.rs:931-940`). The flatness of the Responses form is asserted at `llm.rs:2300-2304`.
...
| Dialect | Shape | Site |
|---|---|---|
| Anthropic | `{type:"image", source:{type:"base64", media_type, data}}` | `llm.rs:748-760` |
| OpenAI Chat | text placeholder in the `role:"tool"` message plus `{type:"image_url", image_url:{url:"data:<mime>;base64,<data>"}}` on a trailing user message | `llm.rs:840-865` |
| Responses | `{type:"input_image", image_url:"data:<mime>;base64,<data>"}` on a trailing user message | `llm.rs:912-925` |

The Chat placeholder text is generated in `openai_tool_text_content` (`llm.rs:840-852`) and reports `data.len()` base64 bytes (`llm.rs:847`).

#### Response parse shapes
| Parser | Reads | Stop derivation | Site |
|---|---|---|---|
| `parse_anthropic` | `content[]` blocks of type `text`, `thinking`, `tool_use` | `map_stop(stop_reason)` (`llm.rs:1155`) | `llm.rs:1154-1196` |
| `parse_openai` | `choices[0].message.{content, reasoning_content, reasoning, tool_calls}` | `map_stop(finish_reason)` (`llm.rs:1198-1204`) | `llm.rs:1197-1247` |
| `parse_responses` | `output[]` items of type `message`, `function_call`, `reasoning` | `status` + `incomplete_details.reason`, plus a `saw_function_call` flag | `llm.rs:991-1091`; stop logic `llm.rs:1064-1079` |

`map_stop` (`llm.rs:1092-1106`) is the shared string→`ProviderStop` table, accepting both dialects' vocabularies for each variant (`end_turn|stop` `llm.rs:1094`, `tool_use|tool_calls` `llm.rs:1095`, max-tokens, `refusal|content_filter` `llm.rs:1097`, else `Other`). `parse_responses` does **not** use `map_stop` — it has its own status mapping (`llm.rs:1064-1079`), so `ProviderStop::Refusal` is unreachable on the Responses path. That asymmetry is untested: grep for `Refusal` in `llm.rs` returned only the `map_stop` definition line (`llm.rs:1097`), no test.

Both `parse_anthropic` and `parse_openai` are *forgiving* about missing fields via `str_field` (`llm.rs:1150-1152`, `unwrap_or("")`), but `parse_openai` hard-errors when `choices` is absent (`llm.rs:1199-1202`), when `message` is absent (`llm.rs:1205-1206`), and when a `tool_calls[]` entry lacks `function` (`llm.rs:1223-1225`).

Nothing in the mesh work changed the parsers. The collective path reuses `parse_openai` unmodified — the mesh gateway answers in Chat Completions shape (`chat_response` fixture, `llm.rs:1756-1763`), and a mesh-specific failure is detected from the *error body* in `post` (`llm.rs:1444-1451`), never in a parser.

#### Reasoning / thinking content
`LlmResponse.reasoning: String` (`types.rs:151`) is filled from three different places:
- Responses: concatenated `summary[].text` on `type == "reasoning"` items, accepting `summary_text` or `text` (`llm.rs:1037-1058`).
- Anthropic: concatenated `thinking` from `type == "thinking"` blocks (`llm.rs:1167-1174`).
- OpenAI Chat: `message.reasoning_content` preferred, falling back to `message.reasoning` (`llm.rs:1208-1218`) — documented as DeepSeek/vLLM extensions.

There are still no tests for any of the three reasoning extraction paths: grep for `reasoning` inside the `llm.rs` test module found only `reasoning_effort` body-shape assertions and one empty `"summary": []` fixture at `llm.rs:2415`.

#### Usage / token accounting
`LlmResponse.input_tokens` and `output_tokens` are `Option<u64>` (`types.rs:139`, `types.rs:145`). `sum_usage` (`llm.rs:1107-1118`) is the single accumulator: it returns `None` when `usage` is absent (`llm.rs:1108`) **or** when it carries none of the requested fields, and otherwise a saturating sum of whichever requested fields are present (`llm.rs:1111-1117`).

| Field | Wire fields summed | Site |
|---|---|---|
| Anthropic input | `input_tokens` + `cache_read_input_tokens` + `cache_creation_input_tokens` | `anthropic_input_tokens`, `llm.rs:1123-1132` |
| OpenAI Chat / legacy Databricks input | `prompt_tokens` + `cache_read_input_tokens` + `cache_creation_input_tokens` | `openai_chat_input_tokens`, `llm.rs:1139-1148` |
| Responses input | `input_tokens` only | `llm.rs:1080` |
| Anthropic output | `output_tokens` | `llm.rs:1186` |
| Chat output | `completion_tokens` | `llm.rs:1237` |
| Responses output | `output_tokens` | `llm.rs:1081` |

The Responses path deliberately does *not* sum cache fields (`llm.rs:1080`), unlike the other two — no comment explains the asymmetry, and no test covers a Responses response that carries cache fields.

Token accounting is well covered: `parse_anthropic_sums_input_and_cache_tokens` (`llm.rs:3450`), `parse_anthropic_input_tokens_only` (`llm.rs:3468`), `parse_anthropic_missing_usage_is_none` (`llm.rs:3478`), `parse_openai_uses_prompt_tokens` (`llm.rs:3487`), `parse_openai_databricks_sums_cache_fields` (`llm.rs:3496`), `parse_openai_missing_usage_is_none` (`llm.rs:3513`), `parse_responses_uses_input_tokens` (`llm.rs:3521`), `parse_responses_missing_usage_is_none` (`llm.rs:3535`), `sum_usage_empty_object_is_none` (`llm.rs:3548`), plus six output-token tests at `llm.rs:3780-3840`. None of the mesh tests assert on usage — the `chat_response` fixture (`llm.rs:1756-1763`) omits `usage` entirely, so a collective response yields `None` for both counters.

#### Byte-budget constants defined in this group
| Constant | Value | Site |
|---|---|---|
| `MAX_LLM_RESPONSE_BYTES` | 16 MiB | `llm.rs:25` |
| `MAX_LLM_ERROR_BODY_BYTES` | 4 KiB | `llm.rs:26` |
| `STALL_NOTICE_THRESHOLD` | 300 s | `llm.rs:27` |
| `MAX_RETRIES` / `BASE_BACKOFF_MS` / `MAX_BACKOFF_MS` | 3 / 500 ms / 8000 ms | `llm.rs:1285-1287` |
...
Body-shape and parse-shape coverage is dense (~75 test fns in `llm.rs:1572-3846`, of which 13 are the new mesh tests at `llm.rs:1826-2199`). What is **not** covered:
- No test constructs a `Config` through `from_env` (it reads process env, so it is untestable without env mutation) — every test builds a `Config` literal (`llm.rs:1579-1610`) or patches `for_discovery` output (`config.rs:1844-1865`). That 27-field literal must be updated by hand whenever a field is added; `16d4ec33` had to add `prefer_mesh_for_auto: false` to it at `llm.rs:1606`.
...
- No test covers `strip_model` (`llm.rs:1560-1570`) — grep for `strip_model` in `llm.rs` returned only the definition (`llm.rs:1560`) and its one call site (`llm.rs:614`).
- `MeshAutoState`'s fields are asserted only *indirectly*, through the sequence of `"model"` values the stub server received (`posted_models`, `llm.rs:1817-1824`). Two tests reach into the state directly, but only to *age* it — `expire_mesh_catalog_check` (`llm.rs:1806-1809`) and `expire_mesh_auto_cooldown` (`llm.rs:1811-1815`) rewrite `last_checked`/`cooldown_until`; no test reads `consecutive_available` or `collective_enabled`.
- `MESH_AUTO_CATALOG_TIMEOUT` (`llm.rs:31`) has no test: the stub server (`llm.rs:1662-1745`) always answers promptly, so the 2 s bound on the probe is never exercised.


## Module: buzz-agent — MCP registry, OAuth, hints & catalog (`crates/buzz-agent/src`)
### Aspect: Data Model

Scope: `mcp.rs` (1,139 lines), `auth.rs` (845), `hints.rs` (726), `builtin.rs` (575), `catalog.rs` (631 — was 402 before `8eb6e3eb`). All types here are in-memory except one on-disk artifact (the OAuth token cache).
...
| Item | Site | Notes |
|---|---|---|
| `ModelEntry` (`Debug, Clone, PartialEq, Eq`) | `catalog.rs:23-27` | `id`, `name`; both set to the endpoint `name` in v1 (`catalog.rs:208-211`) and v2 (`catalog.rs:374-377`) |
| `DATABRICKS_V2_KNOWN_MODELS` | `catalog.rs:32-33` | `["databricks-gpt-5-5", "databricks-claude-opus-4-7"]`, "Mirrors goose's `DATABRICKS_V2_KNOWN_MODELS`" |
| `V2Endpoint` (`Debug, Clone, PartialEq, Eq`, `pub(crate)`) | `catalog.rs:306-313` | New in `8eb6e3eb`. `entry: ModelEntry` (`catalog.rs:309`) + `created_ms: Option<i64>` (`catalog.rs:312`) — the sort key. Exists only between `parse_v2_endpoints_page` and `fetch_v2_models`'s final `map(|endpoint| endpoint.entry)` (`catalog.rs:300-303`), so it never escapes the module |

No struct models the Databricks response; both parsers walk `serde_json::Value` by key (`catalog.rs:172-176`, `catalog.rs:356-360`). Pagination state is a local `Option<String>` page token (`catalog.rs:241`) bounded by a 20-iteration `for` loop (`catalog.rs:245`). The v2 accumulator changed type in `8eb6e3eb` — `Vec<V2Endpoint>` rather than `Vec<ModelEntry>` (`catalog.rs:240`) — so ordering metadata survives pagination and is discarded only after the sort.

#### Parsed v2 endpoint shape (`catalog.rs`)

Fields read off each object in the v2 `endpoints` array, and nothing else:

| Wire field | Accepted JSON shapes | Read at | Mapped to |
|---|---|---|---|
| `name` | string (required — a non-string or absent `name` drops the endpoint via `?`) | `catalog.rs:369` | `entry.id` and `entry.name`, both the same string (`catalog.rs:374-377`) |
| `created_timestamp` | JSON string of epoch milliseconds (the documented gateway shape, `catalog.rs:317-318`) **or** bare JSON number; string values are `trim()`ed before `parse::<i64>()` | `catalog.rs:320-325` | `created_ms: Option<i64>` — `None` on absent, non-numeric, or unparseable (`catalog.rs:310-312`) |

`as_i64()` is tried first and the string parse is the fallback (`catalog.rs:322-324`), so the number shape costs no allocation. Both shapes are pinned by `v2_parse_reads_created_timestamp_in_either_wire_shape` (`catalog.rs:522`), which also asserts that `"not-a-number"` and an absent field both yield `None` (`catalog.rs:535-538`). Negative timestamps are representable (`i64`) and would sort *after* `Some(0)` but still before `None` — nothing rejects them.

The v2 payload has no `task` and no `state.ready` field, unlike v1 (`catalog.rs:85-88`), which is why capability is inferred from `name` alone (see Business Rules). No other wire field is retained anywhere in the file: `grep -n '\.get("' catalog.rs` returns exactly seven keys — `"endpoints"` (`catalog.rs:173`, `:357`), `"name"` (`:185`, `:369`), `"state"`/`"ready"` and `"task"` (`:189-190`, `:200`, v1 only), `"created_timestamp"` (`:321`), `"next_page_token"` (`:384`).

#### Test coverage — Data Model

Covered: `ResultBudget`-driven assembly and both truncation helpers (`tool_result_content_preserves_images` `mcp.rs:1035`, `tool_result_content_elides_images_over_budget` `mcp.rs:1053`, `oversized_text_is_middle_elided` `mcp.rs:1061`, `text_within_budget_is_untouched` `mcp.rs:1082`, `image_passes_whole_even_when_text_budget_is_small` `mcp.rs:1090`, `truncate_middle_respects_max_and_boundaries` `mcp.rs:1107`); `CachedToken` expiry semantics (`cached_token_no_expiry_is_not_expired` `auth.rs:646`, `cached_token_far_future_is_not_expired` `auth.rs:656`, `cached_token_within_leeway_is_expired` `auth.rs:671`); cache path shape (`cache_path_includes_namespace_and_hash` `auth.rs:686`); token-response parsing (`auth.rs:701`, `710`, `716`); `SkillEntry.supporting_files` population (`discover_skills_populates_supporting_files` `hints.rs:702`); `ModelEntry` extraction (`catalog.rs:401`, `442`); and, since `8eb6e3eb`, the `V2Endpoint` shape end-to-end — `created_ms` for both wire variants plus the two `None` cases (`catalog.rs:522`) and the `created_ms`/`entry.name` ordering key (`catalog.rs:542`).
